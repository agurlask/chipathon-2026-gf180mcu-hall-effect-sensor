...
the naming convention given in (FIXME) is used to refer to the four terminal device. The orientation
...
described in (FIXME). Clearly, the symbol shown in Fig.~\ref{fig:hall_plate_bhv} needs an
...
\begin{itemize}
    ...
    \item{
        Design equations in (FIXME) used to estimate bridge resistor values 
...
is stolen from (FIXME), where \textit{sp(x)} clocks show the active phase, 
...
The clock generation circuits from Chapter X of (FIXME) will be used. 
